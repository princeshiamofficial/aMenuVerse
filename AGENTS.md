...
- **Promotion Record Array Mapping & Null-Safety Standard**: When mapping promotion entities fetched from MySQL or `/api/promotions`, client pages (`promotions.tsx`) must normalize all optional array fields (`itemIds`, `categoryNames`, `branchIds`) with explicit array defaults `(Array.isArray(p.itemIds) ? p.itemIds : [])` and alias database columns (`title` -> `name`, `status` -> `active`, `banner_url` -> `image`). JSX count indicators must use safe fallback accessors `(p.itemIds || []).length` to prevent runtime `TypeError: Cannot read properties of undefined (reading 'length')` crashes.
