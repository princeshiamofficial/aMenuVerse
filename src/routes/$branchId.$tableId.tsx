...
export const Route = createFileRoute("/$branchId/$tableId")({
  component: BranchTableRoute,
});

function BranchTableRoute() {
  const { branchId, tableId } = Route.useParams();
  ...
  useEffect(() => {
    async function loadAsync() {
      try {
        const resolved = await resolveTableRestaurantServer({
          data: { token: `${branchId}/${tableId}`, subdomain: detectedSubdomain },
        });
    ...
    loadAsync();
  }, [branchId, tableId, detectedSubdomain]);
