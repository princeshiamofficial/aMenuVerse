...
import type { RestaurantEntry } from "@/lib/admin-context";

export default function AdminLayout({ children }: { children: React.ReactNode }) {
  const [restaurantsList, setRestaurantsList] = useState<RestaurantEntry[]>(RESTAURANTS);
  ...
  useEffect(() => {
    async function loadDbData() {
      try {
        ...
        if (rows) {
          setRestaurantsList(rows as unknown as RestaurantEntry[]);
        }
      } catch (err) {
